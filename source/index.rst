...
NGINX Unit is a polyglot app server,
a reverse proxy,
...
to be highly efficient and fully configurable at runtime.
...
| See the changelog `here </CHANGES.txt>`_
  ...
  outlines our next plans.

| The sources are distributed under the
  `Apache 2.0 <https://hg.nginx.org/unit/file/tip/LICENSE>`_
  license.
| Commercial support is available from
  `NGINX, Inc <https://www.nginx.com/support/>`_.
...
- The
  ...
  is managed dynamically over HTTP via a friendly
  :ref:`RESTful JSON API <configuration-mgmt>`

- Updates to the configuration are performed granularly
  at runtime with zero interruption
...
- :ref:`Java <configuration-java>`:
  using the Servlet Specification 3.1 and WebSocket APIs

- :ref:`Perl <configuration-perl>`:
  using PSGI

- :ref:`PHP <configuration-php>`:
  using a custom SAPI module
